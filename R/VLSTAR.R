...
if (is.null(starting)){
  ...
  if(method == 'ML'){
    ...
    loglike <- function(param){
      gamma <- param[1:((m-1)*ny)]
      c <- param[(ny*(m-1)+1):length(param)]
    ...
    loglike1 <- function(BB, param, Omega){
      ...
      for (z in 1:nrow(y)){
        ...
        Gtilde[[z]] <- t(cbind(In, list.cbind(GT)))
        dify[z] <-  t(y[z, ] - t(Gtilde[[z]])%*%t(BB)%*%x[z,])%*%ginv(Omega)%*%(y[z, ] - t(Gtilde[[z]])%*%t(BB)%*%x[z,])
      }
      sumdif <- sum(dify)
      logll <- -(nrowy*log(det(Omega))/2L) - sumdif/2L  - (nrowy*ncoly/2L)*log(2L*pi)
      return(-logll)
    }
