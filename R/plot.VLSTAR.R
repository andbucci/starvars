...
plot.VLSTAR <- function(object, names = NULL, main.fit = NULL, main.acf = NULL, main.pacf = NULL, main.logi = NULL, ylim.fit = NULL, ylim.resid = NULL, lty.fit = NULL, lty.resid = NULL, lty.logi = NULL, lwd.fit = NULL, lwd.resid = NULL, lwd.logi = NULL, lag.acf = NULL, lag.pacf = NULL, col.fit = NULL, col.resid = NULL, col.logi = NULL,  ylab.fit = NULL, ylab.resid = NULL, ylab.acf = NULL, ylab.pacf = NULL, ylab.logi = NULL, xlab.fit = NULL, xlab.resid = NULL, xlab.logi = NULL, mar = par("mar"), oma = par("oma"), adj.mtext = NA, padj.mtext = NA, col.mtext = NA,...){
  ...
  plotest <- function(y, fitted, resids, logistic, main.fit, main.acf, main.pacf, main.logi, ylab.fit, ylab.resid, ylab.logi,
    ...
    graphics::layout(matrix(c(1, 1, 2, 2, 3, 4, 5, 5), nrow = 4, ncol = 2, byrow = TRUE))
    graphics::par(oma = c(5, 0, 4, 0), mar = c(0, 5.1, 0, 2.1))
    stats::plot.ts(y, main = "", ylim = ylim.fit, ylab = ylab.fit, xlab = xlab.fit, lty = lty.fit[1], lwd = lwd.fit[1], col = col.fit[1], axes = FALSE)
    graphics::lines(fitted, col = col.fit[2], lty = lty.fit[2], lwd = lwd.fit[2])
    graphics::box()
    graphics::axis(2, pretty(c(y, fitted))[-1])
    graphics::mtext(main.fit, side = 3, line = 3, adj = adj.mtext, padj = padj.mtext, col = col.mtext, ...)
    stats::plot.ts(resids, main = "", ylim = ylim.resid, ylab = ylab.resid, xlab = xlab.resid, lty = lty.resid[1], lwd = lwd.resid[1], col = col.resid[1])
    graphics::abline(h = 0, col = col.resid[2], lty = lty.resid[2], lwd = lwd.resid[2])
    graphics::par(mar=c(1, 5.1, 5, 2.1))
    stats::acf(resids, main = main.acf, ylab = ylab.acf, lag.max = lag.acf, ...)
    stats::pacf(resids, main = main.pacf, ylab = ylab.pacf, lag.max = lag.pacf, ...)
    graphics::par(oma = c(6, 0, 5, 0), mar = c(0, 5.1, 0, 2.1))
    graphics::plot(1:length(logistic),logistic, main = main.logi, ylim = c(0,1), ylab = ylab.logi, xlab = xlab.logi, lty = lty.logi[1], lwd = lwd.logi[1], col = col.logi[1], axes = T)
    }
